...
class Connection:
    ...
    def run(self, command, hide:bool=False, pty:bool=False, cwd:str|None = None,**kwargs):
        if cwd:
            command = f"cd {cwd}; {command}"
        output = self.client.run_command(command, use_pty=pty)
        return output

    @contextmanager
    def cd(self, path):
        try:
            new_run = partial(self.run, cwd=path)
            self.run = new_run
            yield self
        finally:
            self.run = self.run
...
@contextmanager
def host_connection(host: HostConfig):
    client = SSHClient(host=host.domain_name, user=host.user, port=host.ssh_port, password=host.password, pkey=host.key_filename.resolve() if host.key_filename else None)
    try:
